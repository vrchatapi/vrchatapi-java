...
public class FileUploadURL {
  ...
  public FileUploadURL url(@javax.annotation.Nonnull String url) {
    this.url = url;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to FileUploadURL
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!FileUploadURL.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in FileUploadURL is not found in the empty JSON string", FileUploadURL.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!FileUploadURL.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `FileUploadURL` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
